fun main() {
    ...
    open class HandBid(val hand: List<Card>, val bid: Int): Comparable<HandBid> {
        ...
        protected open fun isFiveOfAKind(): Boolean {
            return cardCount.containsValue(5)
        }

        protected open fun isFourOfAKind(): Boolean {
        ...
        protected open fun isThreeOfAKind(): Boolean {
            return cardCount.containsValue(3)
        }

        protected open fun isTwoPair(): Boolean {
            return cardCount.count { it.value == 2 } == 2
        }

        protected open fun isOnePair(): Boolean {
    ...
    fun toHand(handStr: String): List<Card> {
        check(handStr.length == 5)
        return handStr.map { Card(it) }
    }
